...
// SEO
useSeoMeta({
  title: 'Create Account | Yndu',
  description: 'Create your Yndu account to start ordering fresh produce from Kibwezi farms.',
});
...
const validate = (): boolean => {
  ...
  if (!phone.value || phone.value.length < 10) {
    errors.value.phone = 'Valid phone number is required (10+ digits)';
  }
  ...
  <div class="min-h-dvh bg-[var(--bg-canvas)] flex items-center justify-center p-4">
    ...
    >
      ...
      <div class="text-center mb-5">
        ...
        <h1 class="text-xl font-bold text-foundation text-balance mb-1">Create your account</h1>
        <p class="text-xs text-neutral-500">Join Yndu for fresh produce delivery</p>
      </div>
        ...
        <div class="p-5">
          ...
          <div class="relative my-4">
            ...
            <div class="relative flex justify-center text-xs">
              <span class="px-3 bg-white dark:bg-zinc-900 text-neutral-400">or create with email</span>
            </div>
          </div>
          ...
          <form @submit.prevent="handleRegister" class="space-y-3">
            ...
            <div>
              ...
              <div class="relative">
                <UserPlus class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" :size="16" />
                <input
                  v-model="name"
                  type="text"
                  ...
                  :class="{ 'border-accent-clay': errors.name }"
                  :disabled="authStore.isLoading"
                />
              </div>
              <p v-if="errors.name" class="mt-1 text-xs text-accent-clay flex items-center gap-1">
            ...
            <div>
              ...
              <div class="relative">
                <Mail class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" :size="16" />
                <input
                  v-model="email"
                  type="email"
                  placeholder="you@example.com"
                  class="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-neutral-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-foundation focus:ring-2 focus:ring-primary-deep/20 focus:border-primary-deep outline-none transition-all"
                  :class="{ 'border-accent-clay': errors.email }"
                  :disabled="authStore.isLoading"
                />
              </div>
              <p v-if="errors.email" class="mt-1 text-xs text-accent-clay flex items-center gap-1">
            ...
            <div>
              ...
              <div class="relative">
                <Phone class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" :size="16" />
                <input
                  v-model="phone"
                  type="tel"
                  placeholder="+254 712 345 678"
                  class="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-neutral-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-foundation focus:ring-2 focus:ring-primary-deep/20 focus:border-primary-deep outline-none transition-all"
                  :class="{ 'border-accent-clay': errors.phone }"
                  :disabled="authStore.isLoading"
                />
              </div>
              <p v-if="errors.phone" class="mt-1 text-xs text-accent-clay flex items-center gap-1">
            ...
            <div>
              ...
              <div class="relative">
                <Lock class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" :size="16" />
                <input
                  v-model="password"
                  ...
                  :class="{ 'border-accent-clay': errors.password }"
                  :disabled="authStore.isLoading"
                />
                <button
                  type="button"
              ...
              <p v-if="errors.password" class="mt-1 text-xs text-accent-clay flex items-center gap-1">
            ...
            <!-- Confirm Password -->
            <div>
              <label class="block text-xs font-medium text-neutral-600 mb-1">Confirm password</label>
              <div class="relative">
                <Lock class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" :size="16" />
                <input
                  ...
                  class="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-neutral-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-foundation focus:ring-2 focus:ring-primary-deep/20 focus:border-primary-deep outline-none transition-all"
                  :class="{ 'border-accent-clay': errors.confirmPassword }"
                  :disabled="authStore.isLoading"
                />
              </div>
              <p v-if="errors.confirmPassword" class="mt-1 text-xs text-accent-clay flex items-center gap-1">
          ...
          <p class="text-center text-xs text-neutral-500 mt-4">
            ...
            <NuxtLink to="/login" class="text-primary-deep font-medium hover:underline">
              Sign in
            </NuxtLink>
          </p>
        </div>
