...
import { MapPin, Plus, Home, Building, Check, ArrowRight } from 'lucide-vue-next';
import { useSubscriptionStore } from '../../stores/subscription-store';
import type { SubscriptionAddress } from '../../types/subscription';

const store = useSubscriptionStore();
const showAddNew = ref(false);

const newAddress = ref<Partial<SubscriptionAddress>>({
  label: '',
  street: '',
  apartment: '',
  neighborhood: '',
...
// Mock saved addresses (would come from API)
const savedAddresses = ref<SubscriptionAddress[]>([
  {
    label: 'Home',
...
const handleAddAddress = () => {
  ...
  store.selectAddress(newAddress.value as SubscriptionAddress);
  showAddNew.value = false;
  newAddress.value = { label: '', street: '', apartment: '', neighborhood: '', city: '', instructions: '' };
};

const handleSelectAddress = (address: SubscriptionAddress) => {
...
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h3 class="text-xl font-bold text-foundation mb-2">Delivery Address</h3>
      <p class="text-sm text-neutral-500">Where should we deliver your subscription boxes?</p>
    </div>

    <!-- Saved Addresses -->
    <div v-if="!showAddNew" class="space-y-3">
      <motion.div
        ...
        :class="[
          store.selectedAddress?.street === address.street
      ...
      >
        <!-- Selection Indicator -->
        <div 
          v-if="store.selectedAddress?.street === address.street"
        ...
        <div class="flex items-start gap-4">
          <div 
            ...
            :class="store.selectedAddress?.street === address.street ? 'bg-primary-deep text-white' : 'bg-neutral-100 dark:bg-zinc-800 text-neutral-500'"
          >
    ...
    <div v-else class="bg-white dark:bg-zinc-900 rounded-xl border border-neutral-100 dark:border-zinc-800 p-6">
      <div class="flex items-center justify-between mb-6">
        <h4 class="font-semibold text-foundation">Add New Address</h4>
        <button 
    ...
    <!-- Continue Button -->
    <div v-if="!showAddNew" class="pt-4">
      <button
        @click="handleContinue"
        :disabled="!store.selectedAddress"
