...
import {
  ...
  useBase,
  useSession,
} from 'file:///app/node_modules/h3/dist/index.mjs';
...
import {
  ...
  withQuery,
  withTrailingSlash,
} from 'file:///app/node_modules/ufo/dist/index.mjs';
import defu, { defu as defu$1, defuFn } from 'file:///app/node_modules/defu/dist/defu.mjs';
import {
  createFetch,
  FetchError,
  Headers as Headers$1,
} from 'file:///app/node_modules/ofetch/dist/node.mjs';
import { snakeCase, upperFirst } from 'file:///app/node_modules/scule/dist/index.mjs';
import {
  createRenderer,
  getPrefetchLinks,
  getPreloadLinks,
  getRequestDependencies,
} from 'file:///app/node_modules/vue-bundle-renderer/dist/runtime.mjs';
import destr, { destr as destr$1 } from 'file:///app/node_modules/destr/dist/index.mjs';
import { createHooks as createHooks$1 } from 'file:///app/node_modules/hookable/dist/index.mjs';
import {
  callNodeRequestHandler,
  fetchNodeRequestHandler,
} from 'file:///app/node_modules/node-mock-http/dist/index.mjs';
...
import { createRouter, toRouteMatcher } from 'file:///app/node_modules/radix3/dist/index.mjs';
...
import {
  ...
  parseRawStackTrace,
} from 'file:///app/node_modules/errx/dist/index.js';
import { isRef, isVNode, toValue } from 'file:///app/node_modules/vue/index.mjs';
...
} from 'file:///app/node_modules/pathe/dist/index.mjs';
import {
  createHead as createHead$1,
  propsToString,
  renderSSRHead,
} from 'file:///app/node_modules/unhead/dist/server.mjs';
...
import { createHooks } from 'file:///app/node_modules/nuxt-auth-utils/node_modules/hookable/dist/index.mjs';
import {
  createIPX,
  createIPXH3Handler,
  ipxFSStorage,
  ipxHttpStorage,
} from 'file:///app/node_modules/@nuxt/image/node_modules/ipx/dist/index.mjs';
...
storage.mount(
  'build',
  unstorage_47drivers_47fs({ 'driver': 'fs', 'readOnly': false, 'base': '/app/.nuxt-docker' }),
);
storage.mount(
  'cache',
  unstorage_47drivers_47fs({
...
function hash(value) {
  return digest(typeof value === 'string' ? value : serialize(value)).replace(/[-_]/g, '').slice(
    0,
    10,
  );
}
...
function defineCachedFunction(fn, opts = {}) {
  ...
  async function get(key, resolver, shouldInvalidateCache, event) {
    const cacheKey = [opts.base, group, name, key + '.json'].filter(Boolean).join(':').replace(
      /:\/$/,
      ':index',
    );
    ...
    const expired = shouldInvalidateCache || entry.integrity !== integrity ||
      ttl && Date.now() - (entry.mtime || 0) > ttl || validate(entry) === false;
...
function defineCachedEventHandler(handler, opts = defaultCacheOptions()) {
  const variableHeaderNames = (opts.varies || []).filter(Boolean).map((h) => h.toLowerCase())
    .sort();
  const _opts = {
    ...opts,
    getKey: async (event) => {
      ...
      const _headers = variableHeaderNames.map((header) => [header, event.node.req.headers[header]])
...
function isJsonRequest(event) {
  ...
  return hasReqHeader(event, 'accept', 'application/json') ||
    hasReqHeader(event, 'user-agent', 'curl/') || hasReqHeader(event, 'user-agent', 'httpie/') ||
    hasReqHeader(event, 'sec-fetch-mode', 'cors') || event.path.startsWith('/api/') ||
...
const errorHandler$0 = async function errorhandler(error, event, { defaultHandler }) {
  ...
  errorObject.url = withoutBase(url.pathname, useRuntimeConfig(event).app.baseURL) + url.search +
    url.hash;
    ...
    if (typeof prettyResponse.body === 'string') {
      return send(
        ...
        html.replace(
          '</body>',
          `${
            generateErrorOverlayHTML(prettyResponse.body, {
              startMinimized: 300 <= status && status < 500,
            })
          }</body>`,
        ),
      );
...
const _EP7owBuMSC_442yxmzv0SJRviHjQkx62Vv0Mi68aE = defineNitroPlugin((nitroApp) => {
  if (
  ...
  nitroApp.hooks.hook('render:html', (htmlContext) => {
    ...
    try {
      ...
      htmlContext.bodyAppend.unshift(
        `<script type="application/json" data-nuxt-logs="${appId}">${
          stringify(ctx.logs, reducers)
        }<\/script>`,
      );
    } catch (e) {
      const shortError = e instanceof Error && 'toString' in e
        ? ` Received \`${e.toString()}\`.`
        : '';
      console.warn(
        `[nuxt] Failed to stringify dev server logs.${shortError} You can define your own reducer/reviver for rich types following the instructions in https://nuxt.com/docs/4.x/api/composables/use-nuxt-app#payload.`,
      );
    }
  });
};
...
const plugins = [
  ...
  _EP7owBuMSC_442yxmzv0SJRviHjQkx62Vv0Mi68aE,
...
const getServerEntry = () =>
  import('file:///app/.nuxt-docker//dist/server/server.mjs').then((r) => r.default || r);
// @ts-expect-error file will be produced after app build
const getClientManifest = () =>
  import('file:///app/.nuxt-docker//dist/server/client.manifest.mjs').then((r) => r.default || r)
...
const getSPARenderer = lazyCachedFunction(async () => {
  ...
  }).then((r) => r.template).catch(() => '').then((r) => {
    {
...
async function getIslandContext(event) {
  ...
  let url = event.path || '';
  const componentParts = url.substring('/__nuxt_island'.length + 1).replace(ISLAND_SUFFIX_RE, '')
    .split('_');
...
function handleMissingConfiguration(event, provider, missingKeys, onError) {
  const environmentVariables = missingKeys.map((key) =>
...
function _useSession(event, config = {}) {
  if (!sessionConfig) {
    ...
    if (!sessionConfig.password) {
      console.error(
        `[nuxt-auth-utils] ${envSessionPassword} environment variable or runtimeConfig.session.password was not set.`,
      );
    }
  }
...
const _UkN92p = eventHandler(async (event) => {
  await clearUserSession(event);
  return { loggedOut: true };
});

const _k40kHK = eventHandler(async (event) => {
...
const _UvzEXr = lazyEventHandler(() => {
...
const handlers = [
  ...
  },
  {
    ...
    handler: _lazy_wD64rD,
    lazy: true,
    middleware: false,
    method: 'post',
  },
  { route: '/auth/google', handler: _lazy_57Klb3, lazy: true, middleware: false, method: 'get' },
  {
    route: '/__nuxt_error',
    handler: _lazy_0XwxzV,
    lazy: true,
    middleware: false,
    method: undefined,
  },
  ...
  { route: '/api/_auth/session', handler: _k40kHK, lazy: false, middleware: false, method: 'get' },
  { route: '/_ipx/**', handler: _UvzEXr, lazy: false, middleware: false, method: undefined },
  { route: '/shop/**', handler: _lazy_0XwxzV, lazy: true, middleware: false, method: undefined },
...
function getSocketAddress() {
  const socketName = `nitro-worker-${process.pid}-${threadId}-${NITRO_DEV_WORKER_ID}-${
...
const template$1 = (messages) => {
  ...
  return '<!DOCTYPE html><html lang="en"><head><title>' + escapeHtml(messages.status) + ' - ' +
    escapeHtml(messages.statusText) + ' | ' + escapeHtml(messages.appName) +
    ...
    escapeHtml(messages.status) + '</h1><h2 class="font-semibold mb-2 sm:text-3xl text-2xl">' +
    escapeHtml(messages.statusText) + '</h2><p class="mb-4 px-2 text-[#64748B] text-md">' +
    escapeHtml(messages.description) + '</p></div></body></html>';
};
...
const login_post = defineEventHandler(async (event) => {
  ...
  try {
    const response = await $fetch(`${denoApiUrl}/api/auth/login`, {
      method: 'POST',
      body: {
        email: body.email,
    ...
    const statusCode = (error == null ? void 0 : error.statusCode) ||
      ((_a = error == null ? void 0 : error.response) == null ? void 0 : _a.status) || 500;
    const message = ((_b = error == null ? void 0 : error.data) == null ? void 0 : _b.message) ||
      (error == null ? void 0 : error.message) || 'Login failed';
    throw createError({
      statusCode,
      message,
    });
...
const register_post = defineEventHandler(async (event) => {
  ...
  const body = await readBody(event);
  if (
    !(body == null ? void 0 : body.email) || !(body == null ? void 0 : body.password) ||
    !(body == null ? void 0 : body.name) || !(body == null ? void 0 : body.phone)
  ) {
    throw createError({
      statusCode: 400,
  ...
  try {
    const response = await $fetch(`${denoApiUrl}/api/auth/register`, {
      method: 'POST',
      body: {
        email: body.email,
        password: body.password,
    ...
    const statusCode = (error == null ? void 0 : error.statusCode) ||
      ((_a = error == null ? void 0 : error.response) == null ? void 0 : _a.status) || 500;
    const message = ((_b = error == null ? void 0 : error.data) == null ? void 0 : _b.message) ||
      (error == null ? void 0 : error.message) || 'Registration failed';
    throw createError({
      statusCode,
      message,
    });
...
const renderer = defineRenderHandler(async (event) => {
  ...
  const isRenderingPayload = (_PAYLOAD_EXTRACTION || routeOptions.prerender) &&
    PAYLOAD_URL_RE.test(ssrContext.url);
...
function renderHTMLDocument(html) {
  return '<!DOCTYPE html>' + `<html${joinAttrs(html.htmlAttrs)}>` +
    `<head>${joinTags(html.head)}</head>` +
    `<body${joinAttrs(html.bodyAttrs)}>${joinTags(html.bodyPrepend)}${joinTags(html.body)}${
      joinTags(html.bodyAppend)
    }</body>` + '</html>';
}
